...
export class Init1664880207341 implements MigrationInterface {
  name = 'Init1664880207341';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`group_to_another_entity\` (\`anotherEntityId\` int NOT NULL, \`groupId\` int NOT NULL, PRIMARY KEY (\`anotherEntityId\`, \`groupId\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`groupId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_e47a8e6c2c4e12f209502140a4\` ON \`group_to_another_entity\` (\`anotherEntityId\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_b40313bc9675a1dcdfbeee03a1\` ON \`group_to_another_entity\` (\`groupId\`)`,
    );
    await queryRunner.query(
